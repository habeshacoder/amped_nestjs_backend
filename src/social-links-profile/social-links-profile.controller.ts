...
export class SocialLinksProfileController {
  constructor(
    private readonly socialLinksProfileService: SocialLinksProfileService,
  ) {}
  ...
  getSellerSocialLinks(@Param('sellerprofile_id') sellerprofile_id: string) {
    return this.socialLinksProfileService.getSellerSocialLinks(
      +sellerprofile_id,
    );
  }
  ...
  update(
    @Param('id') id: string,
    @Body() socialLinksProfileDto: SocialLinksProfileDto,
  ) {
