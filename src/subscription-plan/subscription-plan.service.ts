...
export class SubscriptionPlanService {
    ...
    async create(subscriptionPlanDto: SubscriptionPlanDto) {
        for await (let [i, sub] of subscriptionPlanDto.channel_id.entries()) {//let sub of subscriptionPlanDto.channel_id) {
            try {
    ...
    async getMaterialsInSubPlan(id: number) {
        ...
        if(response) {
            ...
            for(let i = 0; i < response.length; i++) {
                materials[i] = response[i].channelMaterial_id
            }
    ...
    async findForSeller(id: number) {
        ...
        for await (let chan of channel) {
            subs.push(await this.prisma.subscriptionPlan.findMany({
                where: {
                    channel_id: chan.id,
                },
                include: {
    ...
    async remove(id: number) {
        ...
        if (subscriptionPlan.material_in_subscription_plan != null) {
            ...
            return {'message': "Subscription Plan has files inside. please remove them first to delete the plan."};
        }
    }
}