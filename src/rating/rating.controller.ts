...
export class RatingController {
    ...
    myMaterialReview(@Param('material_id') materialId: string, @GetUser() user: User) {
        return this.ratingService.getMyMaterialReview(+materialId, user)
    }

    @UseGuards(JwtGuard)
    @Get('my_channel_review/:channel_id')
    myChannelReview(@Param('channel_id') channelId: string, @GetUser() user: User) {
    ...
    noOfMaterialRating(@Query('rating') rating: string, @Query('material_id') material_id: string) {
        return this.ratingService.noOfMaterialRating({rating: Number(rating), material_id: Number(material_id)});
    }

    @Get('rating/channel_rating')
    noOfChannelRating(@Query('rating') rating: string, @Query('channel_id') channel_id: string) {
        return this.ratingService.noOfChannelRating({rating: Number(rating), channel_id: Number(channel_id)});
    }
